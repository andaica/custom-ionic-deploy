...
- (void) check:(CDVInvokedUrlCommand *)command {
    ...
    if([command.arguments count] > 2 && [command.arguments objectAtIndex:2] != NULL){
        self.server_url = [command.arguments objectAtIndex:2];
    }
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        if(result.json != nil) {
            ...
            NSString *binary_version = [result.json objectForKey:@"binary_version"];
            ...
            NSNumber* yes_number = [NSNumber numberWithBool:YES];

            if (![compatible isEqual:yes_number] || [binary_version isEqualToString:app_version]) {
                NSLog(@"Refusing update due to incompatible binary version");
            } else if([update_available isEqual:yes_number]) {
            ...
            if ([update_available isEqual:yes_number] && ![binary_version isEqualToString:app_version]) {
...
- (void) download:(CDVInvokedUrlCommand *)command {
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        if (false && upstream_uuid != nil && [self hasVersion:upstream_uuid]) {
...
- (struct JsonHttpResponse) postDeviceDetails2 {
...
- (struct JsonHttpResponse) postDeviceDetails {
    ...
    NSString *baseUrl = self.server_url;
    NSString *endpoint = [NSString stringWithFormat:@"/autoupdate/%@/version.json?time=%@", self.channel_tag, timeStampObj];
    NSString *url = [NSString stringWithFormat:@"%@%@", baseUrl, endpoint];
    NSDictionary* headers = @{@"Content-Type": @"application/json", @"accept": @"application/json"};
    NSString *uuid = [[NSUserDefaults standardUserDefaults] objectForKey:@"uuid"];
    NSString *app_version = [[self deconstructVersionLabel:self.version_label] firstObject];
    ...
    NSLog(@"-> response is: %@", [result.body JSONObject]);
